...
if [ -n "$(find "$CT_CONFIG_DIR" -mindepth 1 -maxdepth 1 -type f 2>/dev/null)" ]; then
  /bin/consul-template -consul-addr="$CONSUL_ADDR" -consul-ssl -vault-addr="$VAULT_ADDR" -vault-ssl -vault-renew-token="false" -config="$CT_CONFIG_DIR" -once -log-level="debug"
else
